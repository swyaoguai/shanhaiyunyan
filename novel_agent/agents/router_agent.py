...
_USER_VISIBLE_AGENT_NAME_REPLACEMENTS = {
    ...
    "PolisherAgent": "正文润色师",
    "Polisher": "正文润色师",
    "EvaluatorAgent": "质检评估师",
    "Evaluator": "质检评估师",
    "CommunicatorAgent": "创作沟通助手",
    "Communicator": "创作沟通助手",
    "Coordinator": "创作协调器",
    "Router": "智能路由助手",
    "WebSearch": "网络搜索助手",
...
【Router职责】
- 只识别真实意图、选择目标流程、形成交接；不要生成创作内容。
- 不把系统进度、Agent 名称、任务日志、按钮文案、错误提示当成用户设定。
- 只有用户明确要求生成/创建/保存/写入/开始/续写/润色/重写/查询/搜索时，才判为执行类意图。
- 用户询问 Agent、提示词、配置、流程、原因、能力边界、是否可以时，优先 GENERAL_CHAT 或 CONFIG_SETTINGS。

【创作助手职责】
- 世界观设定师：构建世界观（世界设定、力量体系、地理、势力、文化等）
- 角色构建师：创建角色档案/人设卡（主角、配角、反派等人物）
- 全书大纲规划师：规划全书大纲（卷级结构、主线/支线剧情）
- 章节正文写手：撰写章节正文
- 质检评估师：评估章节质量
- 正文润色师：润色修订章节
- 无限续写正文写手：无限续写（对已有章节续写）
...
- "怎么写/要不要/是否/帮我看看/先讨论/为什么/哪里有问题" → GENERAL_CHAT 或 ASK_HELP，不要判成执行类创作意图
- "提示词/Agent配置/模型/API/权限/能否自定义" → CONFIG_SETTINGS 或 GENERAL_CHAT，不要判成创作任务
- "消息传递/意图识别/多Agent闭环/上下文共享" → GENERAL_CHAT，除非用户明确要求修改配置或开始生成项目资料"""
        ...
        return (
            ...
            "5. 提示词、Agent 配置、模型/API 设置、权限、自定义能力相关请求优先 CONFIG_SETTINGS；若只是问原因或讨论改造方案，优先 GENERAL_CHAT。\n"
            "6. CREATE_* 只表示用户主题属于创作任务；是否立即写入由后续执行门控判断，不要为了“可能会写”而提高置信度。\n"
            "7. 不要把系统日志、进度消息、Agent 名称、错误文本拆成创作意图。\n"
            "8. 置信度：很确定用0.9+，比较确定用0.7-0.9，不确定用0.5-0.7。\n"
            "9. 如果用户只表达了一个意图，数组中只返回一个元素。\n"
            "10. original_text 是该意图对应的原始用户文本片段。\n\n"
            "只返回 JSON 数组，不要解释：\n"
            '[\n'
            '  {\n'
...
- "消息传递/意图识别/多Agent闭环/上下文共享" → GENERAL_CHAT，除非用户明确要求修改配置或开始生成项目资料"""
    ...
    def _record_tool_call_usage(
        ...
        """Record non-LLM tool invocations so call counts include tools."""
        try:
            ...
            payload_out = json.dumps(result or {"error": error}, ensure_ascii=False, default=str)
            record_token_usage(
                agent_name=self.name,
                model=str(tool_name or "tool"),
                tokens_in=estimate_tokens_from_text(payload_in),
                tokens_out=estimate_tokens_from_text(payload_out),
                success=bool(success),
                method=f"tool_call:{tool_name or 'unknown'}",
                duration=0,
            )
        except Exception as exc:
            logger.debug(f"[{self.name}] record tool usage skipped: {exc}")
            ...
            if tasks:
                ...
                for i, (task_type, task_result) in enumerate(zip([t[0] for t in tasks], task_results)):
                    if isinstance(task_result, Exception):
                        if task_type == "tool":
                            self._record_tool_call_usage(first_intent.tool_name, first_intent.tool_args or {}, None, False, str(task_result))
                        ...
                        if task_type == "kb":
                            ...
                            self._record_tool_call_usage(
                                first_intent.tool_name,
                                first_intent.tool_args or {},
                                task_result,
                                bool(task_result and task_result.get("success")),
                                str(task_result.get("error") or "") if isinstance(task_result, dict) else "",
                            )
    ...
    def _derive_chapters_for_target_words(target_word_count: int) -> int:
        if target_word_count <= 0:
            return 0
        return max(5, min(MAX_AUTO_DERIVED_CHAPTERS, int(math.ceil(target_word_count / 3000))))
    ...
    def _extract_local_creation_requirement_hints(message: str) -> Dict[str, Any]:
        ...
        volume_match = re.search(r"(?:分成|分为|分|共|大概分成|大致分成)\s*(\d{1,3})\s*卷", text)
        if volume_match:
            hints["volume_count"] = max(1, int(volume_match.group(1)))
        ...
        if target_words:
            ...
            if per_chapter_words:
                derived_total_chapters = max(
                    1,
                    min(MAX_AUTO_DERIVED_CHAPTERS, int(math.ceil(target_words / per_chapter_words))),
                )
                hints["total_chapters"] = derived_total_chapters
                hints["chapters_per_volume"] = derived_total_chapters
            else:
    ...
    def _merge_creation_requirement_hints(
        ...
        for key in ("target_word_count", "target_words_per_chapter", "total_chapters"):
            if hints.get(key):
    ...
    @staticmethod
    def _is_planning_only_creation_request(message: str) -> bool:
        text = str(message or "").strip()
        if not text:
            return False
        planning_markers = (
        ...
        return any(marker in text for marker in planning_markers) and not any(
            marker in text for marker in execution_markers
        )
    ...
    def _build_creation_contract_payload(
        ...
        *,
        ...
        contract = build_default_creation_contract(
            ...
            total_chapters=self._normalize_positive_int(requirements.get("total_chapters"), 0),
        ...
        if self._is_planning_only_creation_request(str(requirements.get("source_message") or "")):
            contract.metadata["planning_only"] = True
            contract.metadata["pause_after_outline"] = True
    ...
    @staticmethod
    def _project_state_mtime(pm: Any, state_key: str) -> float:
        try:
            path = pm.get_project_state_path(state_key)
            if path.exists():
    ...
    @staticmethod
    def _project_data_mtime(pm: Any, data_type: str) -> float:
        try:
            path = pm.get_project_data_path(data_type)
            if path.exists():
                return path.stat().st_mtime
        except Exception:
            return 0.0
    ...
    def _project_outline_is_stale_for_contract(self, pm: Any) -> bool:
        outline_mtime = self._project_data_mtime(pm, "outline")
        if outline_mtime <= 0:
            return False
        newest_planning_mtime = max(
            self._project_state_mtime(pm, "creation_contract"),
            self._project_state_mtime(pm, "task_pool"),
        )
        return newest_planning_mtime > outline_mtime + 1

    @staticmethod
    def _project_creation_contract_scope(pm: Any) -> Dict[str, Any]:
        try:
    ...
    @staticmethod
    def _context_with_creation_contract_scope(
        context: Optional[Dict[str, Any]],
        scope: Dict[str, Any],
    ) -> Optional[Dict[str, Any]]:
        if not scope:
        ...
        creation_requirements = dict(scope)
        existing_requirements = enriched.get("creation_requirements")
        if isinstance(existing_requirements, dict):
            for key, value in existing_requirements.items():
                if value not in (None, "", [], {}):
                    creation_requirements[key] = value
        enriched["creation_requirements"] = creation_requirements
        return enriched
        ...
        *,
        ...
        if not content:
            return {
                "agent_name": "Polisher",
                "action": "polish",
                "response": "好的，请把需要润色的正文直接贴给我，我会立即交给正文润色师处理。",
                "is_complete": False,
                "run_id": self._get_run_id(context),
            }
    ...
    async def _load_outline_rows_with_auto_generation(
        ...
        reused_files: List[Dict[str, str]] = []
        outline_rows = pm.load_project_data("outline")
        should_refresh_outline = self._project_outline_is_stale_for_contract(pm)
        if (
            isinstance(outline_rows, list)
            and any(isinstance(row, dict) for row in outline_rows)
            and not should_refresh_outline
        ):
            return [row for row in outline_rows if isinstance(row, dict)], created_files, updated_files, reused_files, None

        if should_refresh_outline:
            await self._emit_progress(context, {
                "content": "### 大纲阶段\n检测到创作规划已更新，正在先重建大纲，避免复用旧资料生成章纲...",
                "current_agent": "Outliner",
                "stage": "outlining",
                "status": "running",
                "created_files": created_files,
        ...
        if should_refresh_outline:
            outline_context = self._context_with_creation_contract_scope(
                context,
                self._project_creation_contract_scope(pm),
            )

        outline_result = await self._execute_outline_pipeline(message=message, context=outline_context)
        ...
        if not isinstance(outline_rows, list):
            outline_rows = []
        outline_rows = [row for row in outline_rows if isinstance(row, dict)]
        if isinstance(outline_result, dict) and outline_rows:
            try:
                outline_path = str(pm.get_project_data_path("outline"))
            except Exception:
                outline_path = ""
            has_outline_record = any(
                isinstance(item, dict)
                and (
                    item.get("kind") == "outline"
                    or (outline_path and str(item.get("path") or "").strip() == outline_path)
                )
                for item in created_files + updated_files + reused_files
            )
            if outline_path and not has_outline_record:
                self._append_file_record_by_status(
                    file_record=self._build_file_record(outline_path, "outline", "大纲", "updated"),
                    created_files=created_files,
                    updated_files=updated_files,
                    reused_files=reused_files,
                )
