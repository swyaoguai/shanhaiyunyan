...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="小说创作Agent",
        description="基于多Agent协作的智能小说创作系统",
        version="1.0.0",
        lifespan=lifespan
    )
    ...
    def get_resource_path(relative_path: str) -> Path:
        ...
        if getattr(sys, 'frozen', False):
            ...
            # 尝试多个可能的位置
            possible_paths = [
                base_path.parent / "resources" / relative_path,  # 便携版结构
                base_path / relative_path,  # 直接在app目录下
                base_path / "_internal" / "novel_agent" / "web" / relative_path,  # PyInstaller默认
            ]
...
".join(lines), encoding="utf-8")
    ...
    async def start_continuous_write(request: ContinuousWriteStartRequest):
        ...
        # 创建Agent实例
        writer = ContinuousWriter(write_config=write_config, model_config=model_config)
        
        # 尝试获取知识库实例（不使用模拟，必须配置真实向量化服务）
        pm = get_project_manager()
        if pm.current_project_id:
            try:
                ...
                # 知识库不可用，继续但不存储
                import logging
                logging.getLogger(__name__).warning(f"知识库初始化失败: {e}")
    ...
    async def get_trend_platforms():
        ...
        platforms = [
            ...
            },
            {
                ...
                "tool": "get-bilibili-trending"
            },
            {
                "id": "baidu",
                "name": "百度热搜",
                "icon": "ri-baidu-fill",
                "description": "获取百度实时热搜",
                "tool": "get-baidu-trending"
            },
            {
                "id": "toutiao",
                "name": "头条热榜",
            ...
            },
            {
                ...
                "tool": "get-github-trending"
            },
            {
                "id": "hackernews",
                "name": "HackerNews",
                "icon": "ri-global-fill",
                "description": "获取HackerNews热门",
                "tool": "get-hackernews-trending"
            }
        ]
...
# 模块职责说明：提供FastAPI Web应用，包含小说创作、项目管理、API配置、Letta集成等所有HTTP接口。
